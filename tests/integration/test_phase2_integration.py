...
class TestPhase2Integration:
    ...
    async def test_command_executor_real(self, browser_session):
        ...
        try:
            ...
            logger.info(f"Navigation command executed successfully in {response.execution_time:.3f}s")
    ...
    async def test_concurrent_operations(self, browser_session):
        ...
        try:
            ...
            logger.info(f"Concurrent operations: {len(successful_results)}/{len(commands)} succeeded")
    ...
    async def test_phase2_end_to_end(self):
        ...
        try:
            async with websockets.connect("ws://localhost:9877") as websocket:
                ...
                summary_message = LLMMessage(command=summary_command, session_id="e2e-session")
    ...
    async def test_navigation_latency(self):
        ...
        try:
            ...
            for i in range(5):
                nav_command = NavigateCommand(
                    url="https://httpbin.org/html",
                    browser_id="perf-browser"
                )
                
                start_time = asyncio.get_event_loop().time()
                response = await executor.execute_command(nav_command, f"perf-session-{i}")
            ...
            if nav_times:
                ...
                logger.info(f"Average navigation time: {avg_time:.3f}s ({len(nav_times)} samples)")
