...
from surfboard.automation.smart_waiter import SmartWaiter, WaitType, WaitResult
...
class TestAdvancedElementSelector:
    ...
    @pytest.mark.asyncio
    async def test_exact_match_strategy_success(self, selector, mock_session, element_selector_css):
    ...
    @pytest.mark.asyncio
    async def test_fuzzy_text_strategy(self, selector, mock_session, element_selector_text):
        ...
        result = await selector._fuzzy_text_strategy(mock_session, element_selector_text)
    ...
    @pytest.mark.asyncio
    async def test_context_aware_strategy(self, selector, mock_session, element_selector_css):
    ...
    @pytest.mark.asyncio
    async def test_multiple_elements_selection(self, selector, mock_session, element_selector_css):
        ...
        results = await selector.find_multiple_elements(mock_session, element_selector_css, limit=5)
        
        assert len(results) == 2
        assert all(result.strategy == SelectionStrategy.EXACT_MATCH for result in results)
    ...
    @pytest.mark.asyncio
    async def test_element_not_found(self, selector, mock_session, element_selector_css):
...
class TestSmartWaiter:
    ...
    @pytest.mark.asyncio
    async def test_wait_for_element_visible_success(self, waiter, mock_session, element_selector):
        ...
        condition_met, info = await waiter._check_element_visible(mock_session, element_selector)
    ...
    @pytest.mark.asyncio
    async def test_wait_for_element_clickable(self, waiter, mock_session, element_selector):
        ...
        # Test the internal method directly
        condition_met, info = await waiter._check_element_clickable(mock_session, element_selector)
...
class TestErrorRecoverySystem:
    ...
    @pytest.mark.asyncio
    async def test_immediate_retry_strategy(self, recovery_system, mock_session, test_command):
    ...
    @pytest.mark.asyncio
    async def test_exponential_backoff_strategy(self, recovery_system, mock_session, test_command):
    ...
    @pytest.mark.asyncio
    async def test_page_refresh_strategy(self, recovery_system, mock_session, test_command):
    ...
    @pytest.mark.asyncio
    async def test_retry_with_recovery(self, recovery_system, mock_session, test_command):
    ...
    @pytest.mark.asyncio
    async def test_graceful_degradation(self, recovery_system, mock_session, test_command):
        ...
        success, result = await recovery_system._graceful_degradation(mock_session, error_context)
    ...
    def test_recovery_pattern_learning(self, recovery_system):
        ...
        assert recovery_system.error_history[0]["strategy"] == RecoveryStrategy.ELEMENT_WAIT_RETRY
...
class TestPerformanceOptimizer:
    ...
    async def test_performance_metrics_collection(
        self, mock_net_connections, mock_cpu_percent, mock_virtual_memory, optimizer, mock_session
    ):
    ...
    def test_different_optimization_levels(self):
        ...
        conservative_strategy = conservative_optimizer.optimization_strategies[OptimizationLevel.CONSERVATIVE]
        aggressive_strategy = aggressive_optimizer.optimization_strategies[OptimizationLevel.AGGRESSIVE]
        maximum_strategy = maximum_optimizer.optimization_strategies[OptimizationLevel.MAXIMUM]
        
        # More aggressive levels should have shorter cleanup intervals
        assert conservative_strategy["dom_cleanup_interval"] > aggressive_strategy["dom_cleanup_interval"]
        assert aggressive_strategy["dom_cleanup_interval"] > maximum_strategy["dom_cleanup_interval"]
...
class TestAdvancedInteractionEngine:
    ...
    @pytest.mark.asyncio
    async def test_hover_element(self, interaction_engine, mock_session, element_selector):
    ...
    async def test_drag_and_drop(self, interaction_engine, mock_session):
        ...
        mock_session.runtime.evaluate.side_effect = [
            ...
            {
                "found": True, 
                "bounds": {"x": 300, "y": 400, "width": 50, "height": 50}
            },
            # Page state before
    ...
    @pytest.mark.asyncio
    async def test_scroll_into_view(self, interaction_engine, mock_session, element_selector):
    ...
    @pytest.mark.asyncio
    async def test_element_not_found_error(self, interaction_engine, mock_session, element_selector):
        """Test handling when element is not found."""
...
async def test_integration_advanced_automation():
    ...
    optimizer = PerformanceOptimizer(OptimizationSettings(level=OptimizationLevel.MODERATE))
...
class TestPerformanceStress:
    ...
    async def test_large_number_of_elements(self):
        ...
        for i in range(1000):
            large_element_list.append({
                ...
                "boundingBox": {"x": i % 100, "y": i // 100, "width": 50, "height": 20},
                "isVisible": True
            })
        ...
        results = await selector.find_multiple_elements(mock_session, element_selector, limit=50)
    ...
    async def test_concurrent_operations(self):
        ...
        waiter = SmartWaiter()
        mock_session = AsyncMock()
        mock_session.runtime.evaluate = AsyncMock(return_value={"visible": True, "found": True})
