    ...
    try:
        ...
        async with websockets.connect("ws://localhost:8765") as websocket:
            ...
            nav_command = NavigateCommand(
                url="https://httpbin.org/forms/post", browser_id="demo-browser"
            )

            message = LLMMessage(command=nav_command, session_id="demo-session")
            ...
            response_data = json.loads(response)
            logger.info(
                f"Find element response: {response_data['response']['element_found']}"
            )
            ...
            commands = [
                ...
                TypeTextCommand(
                    selector=ElementSelector(
                        type=ElementSelectorType.CSS, value="input[name='custemail']"
                    ),
            ...
            for cmd in commands:
                ...
                response_data = json.loads(response)
                logger.info(
                    f"Form fill response: {response_data['response']['status']}"
                )

            # Test screenshot
            screenshot_command = TakeScreenshotCommand(
                format="png", browser_id="demo-browser"
            )
            ...
            if response_data["response"]["status"] == "success":
                ...
                logger.info(
                    f"Page analysis: {page_info['title']} - {page_info['word_count']} words"
                )
                logger.info(
                    f"Found {len(page_info['forms'])} forms, {len(page_info['links'])} links"
                )
...
async def demo_command_protocol():
    ...
    commands = [
        NavigateCommand(url="https://example.com", timeout=15.0),
        FindElementCommand(
            selector=ElementSelector(type=ElementSelectorType.TEXT, value="Click here"),
            multiple=False,
        ),
        ClickCommand(
            selector=ElementSelector(
                type=ElementSelectorType.CSS, value="button.submit"
            ),
            button="left",
...
async def demo_page_analysis():
    ...
    async with managed_browser(headless=True) as browser:
        ...
        interactive_elements = await analyzer.get_interactive_elements(
            session, limit=10
        )
        ...
        for i, field in enumerate(form_fields[:3]):
            logger.info(
                f"  {i+1}. {field['type']} field: {field.get('name', 'unnamed')} - {field.get('placeholder', '')}"
            )
...
async def demo_error_handling():
    ...
    from surfboard.protocols.llm_protocol import StatusType, create_error_response
    ...
    try:
        ...
        invalid_nav = NavigateCommand(
            url="http://invalid-domain-12345.fake", timeout=5.0
        )
        ...
        invalid_find = FindElementCommand(
            selector=ElementSelector(
                type=ElementSelectorType.CSS, value="#nonexistent-element-12345"
            ),
            timeout=2.0,
        )
...
async def main():
    ...
    try:
        ...
        if schemas_dir.exists():
            ...
            logger.info(
                f"Generated {schema_count} JSON schema files in 'schemas/' directory"
            )
