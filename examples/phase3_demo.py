...
async def demo_advanced_element_selection():
    ...
    try:
        ...
        css_selector = ElementSelector(
            type=ElementSelectorType.CSS, value="input[name='custname']"
        )

        result = await selector.find_element(session, css_selector)
        if result:
            logger.info(
                f"Found element using {result.strategy.value}: {result.selector_used}"
            )
        ...
        text_selector = ElementSelector(
            type=ElementSelectorType.TEXT, value="Customer name"
        )

        result = await selector.find_element(session, text_selector)
        if result:
            logger.info(
                f"Found element using {result.strategy.value}: confidence {result.confidence:.2f}"
            )
        else:
        ...
        phone_selector = ElementSelector(
            type=ElementSelectorType.CSS, value="input[name='custtel']"
        )

        result = await selector.find_element(session, phone_selector, context=context)
        if result:
            logger.info(
                f"Context-aware selection successful: {result.element_id or 'no-id'}"
            )

        # Test multiple element selection
        logger.info("Testing multiple element selection...")
        input_selector = ElementSelector(type=ElementSelectorType.CSS, value="input")

        results = await selector.find_multiple_elements(
            session, input_selector, limit=5
        )
        logger.info(f"Found {len(results)} input elements")
        for i, result in enumerate(results[:3]):
            logger.info(
                f"  {i+1}. {result.attributes.get('name', 'unnamed')} - {result.text_content[:30]}"
            )
...
async def demo_smart_waiting():
    ...
    try:
        ...
        complex_selector = ElementSelector(
            type=ElementSelectorType.CSS, value="nonexistent-element"
        )
        ...
        logger.info(
            f"Adaptive timeout test: {result.condition_met} in {result.wait_time:.2f}s"
        )
...
async def demo_error_recovery():
    ...
    try:
        ...
        test_command = BaseCommand(
            command_type=CommandType.CLICK, browser_id="demo-browser"
        )
...
async def demo_performance_optimization():
    ...
    for level in optimization_levels:
        ...
        try:
            ...
            logger.info(
                f"  Page load optimization time: {optimization_result['total_optimization_time']:.3f}s"
            )
            logger.info(
                f"  Load metrics: {optimization_result['load_metrics'].get('total_time', 0):.3f}s"
            )
            ...
            logger.info(
                f"  Script execution time: {script_result['execution_time']:.3f}s"
            )
            ...
            logger.info(
                f"  Cache entries cleared: {cleanup_result['cache_entries_cleared']}"
            )
    ...
    try:
        ...
        if hover_result.success:
            ...
            if hover_result.state_changes:
                logger.info(
                    f"State changes detected: {list(hover_result.state_changes.keys())}"
                )
        else:
        ...
        submit_selector = ElementSelector(
            type=ElementSelectorType.CSS, value="input[type='submit']"
        )
        ...
        if swipe_result.success:
            logger.info(
                f"Swipe gesture completed: {swipe_result.element_info['direction']}"
            )
        ...
        if pinch_result.success:
            logger.info(
                f"Pinch gesture completed: {pinch_result.element_info['zoom_type']}"
            )
        ...
        logger.info(
            f"Workflow completed: {successful_steps}/{len(workflow.steps)} steps successful"
        )

        for i, result in enumerate(workflow_results):
            logger.info(
                f"  Step {i+1}: {result.interaction_type.value} - {'✓' if result.success else '✗'}"
            )
...
async def demo_integration_showcase():
    ...
    try:
        ...
        logger.info(
            f"1. Optimized page load: {optimization_result['total_optimization_time']:.3f}s"
        )
        ...
        if selection_result:
            logger.info(
                f"3. Element found: {selection_result.strategy.value} (confidence: {selection_result.confidence:.2f})"
            )
        else:
            ...
            if selection_result:
                logger.info(
                    f"3. Alternative selection successful: {selection_result.strategy.value}"
                )
        ...
        if selection_result:
            ...
            if wait_result.success:
                logger.info(
                    f"4. Element ready for interaction: {wait_result.wait_time:.3f}s"
                )
                ...
                if hover_result.success:
                    logger.info(
                        f"4. Hover interaction successful: {hover_result.execution_time:.3f}s"
                    )
                else:
                    ...
                    recovery_result = await recovery_system.handle_error(
                        session,
                        Exception(hover_result.error_message or "hover failed"),
                        test_command,
                        attempt_count=1,
                    )

                    logger.info(
                        f"4. Recovery strategy: {recovery_result.strategy_used.value}"
                    )
        ...
        logger.info(
            f"   Cache cleared: {cleanup_result['cache_entries_cleared']} entries"
        )
        ...
        test_command = BaseCommand(
            command_type=CommandType.NAVIGATE, browser_id="demo-browser"
        )
...
async def main():
    ...
    logger.info("=" * 80)
    logger.info(
        f"Phase 3 demonstrations completed: {successful_demos}/{len(demos)} successful"
    )
    ...
    logger.info(
        "✓ Enhanced element selection with 6 strategies and fallback mechanisms"
    )
