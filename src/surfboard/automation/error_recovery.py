...
from ..protocols.llm_protocol import BaseCommand, BaseResponse, StatusType, ElementSelector
...
class ErrorRecoverySystem:
    ...
    async def handle_error(
        ...
        logger.warning(f"Handling error: {error_context.error_type} (attempt {attempt_count})")
        ...
        for strategy in strategies:
            try:
                ...
                if success:
                    ...
                    self._record_successful_recovery(error_context, strategy, recovery_time)
    ...
    async def retry_with_recovery(
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                ) + random.uniform(0, 0.5)  # Add jitter
    ...
    def _get_recovery_strategies(self, error_context: ErrorContext) -> List[RecoveryStrategy]:
        ...
        strategies = self.strategy_map.get(error_context.error_type, [
            RecoveryStrategy.EXPONENTIAL_BACKOFF,
            RecoveryStrategy.GRACEFUL_DEGRADATION
        ]).copy()
    ...
    async def _generate_alternative_selectors(
        self,
        session: CDPSession,
        original_selector: ElementSelector
    ) -> List[Dict[str, str]]:
    ...
    def _get_learned_strategies(self, error_context: ErrorContext) -> List[RecoveryStrategy]:
        """Get strategies that have worked for similar errors."""
        
        # Filter history for similar errors
        similar_errors = [
            entry for entry in self.error_history[-50:]  # Last 50 errors
            if (entry.get("error_type") == error_context.error_type and
                entry.get("success", False))
        ]
        
        # Extract successful strategies
        strategies = [
            entry.get("strategy") for entry in similar_errors
