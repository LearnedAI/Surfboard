...
class SmartWaiter:
    ...
    async def wait_for_condition(
        ...
        if adaptive:
            ...
            logger.debug(f"Adaptive timeout: {adjusted_timeout:.1f}s (original: {timeout:.1f}s)")
                ...
                if condition_met:
                    ...
                    if stability_time > 0:
                        ...
                        if time.time() - stability_start >= stability_time:
                            ...
                            logger.debug(f"Condition met after {wait_time:.2f}s with stability")
        ...
        # Timeout occurred
        wait_time = time.time() - start_time
        logger.warning(f"Wait timeout after {wait_time:.2f}s for condition: {condition}")
    ...
    async def _check_element_stable(
        ...
        position_change = abs(first_measurement["x"] - second_measurement["x"]) + \
                         abs(first_measurement["y"] - second_measurement["y"])
        
        size_change = abs(first_measurement["width"] - second_measurement["width"]) + \
    ...
    async def _check_page_load(
        ...
        loaded = (result.get("readyState") == "complete" and
                 result.get("pendingImages", 0) == 0 and
                 result.get("pendingScripts", 0) == 0)
    ...
    def _get_historical_factor(self, condition: WaitType) -> float:
        ...
        condition_history = [
            entry for entry in self.wait_history[-50:]  # Last 50 entries
        ...
        successful_waits = [
            entry["wait_time"] for entry in condition_history
