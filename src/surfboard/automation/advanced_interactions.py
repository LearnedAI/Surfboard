...
class AdvancedInteractionEngine:
    ...
    async def hover_element(
        ...
        try:
            ...
            await session.input.dispatch_mouse_event(
                type="mouseMoved", x=hover_x, y=hover_y
            )
    ...
    def _compare_states(
        ...
        if "bounds" in before and "bounds" in after:
            ...
            for bound_key in ["x", "y", "width", "height"]:
                if (
                    abs(
                        before["bounds"].get(bound_key, 0)
                        - after["bounds"].get(bound_key, 0)
                    )
                    > 1
                ):
                    bounds_changed = True
    ...
    async def _perform_swipe(
        ...
        # End touch
        await session.input.dispatch_touch_event(type="touchEnd", touchPoints=[])
    ...
    async def _perform_pinch(
        ...
        final_distance = (
            ...
            if not end_point
                ...
                (end_point.x - center_point.x) ** 2
                + (end_point.y - center_point.y) ** 2
            )
            ** 0.5
        )
    ...
    async def _perform_tap(
        self, session: CDPSession, point: InteractionPoint
    ) -> InteractionResult:
        """Perform tap gesture."""
    ...
    async def _perform_long_press(
        self, session: CDPSession, point: InteractionPoint, duration: float
    ) -> InteractionResult:
    ...
    async def _execute_workflow_step(
        self, session: CDPSession, step: Dict[str, Any]
    ) -> InteractionResult:
        ...
        if step_type == "hover":
            ...
            return await self.drag_and_drop(
                ...
                ElementSelector(**step["target_selector"])
    ...
    async def _check_condition(self, session: CDPSession, condition: str) -> bool:
    ...
    async def _rollback_workflow(
        self, session: CDPSession, workflow: InteractionSequence, failed_step: int
    ) -> None:
