...
class BrowserInstance:
    ...
    def __init__(
        ...
        self.chrome_manager = manager  # Alias for backward compatibility
...
class BrowserProfile:
    ...
    def create_persistent(
        cls, name: str, base_dir: Optional[Path] = None
    ) -> "BrowserProfile":
...
class BrowserManager:
    ...
    async def list_instances(self) -> List[Dict[str, Any]]:
        ...
        for instance_id, instance in self.instances.items():
            info = {
                ...
                "profile_path": str(instance.profile_path)
    ...
    async def create_browser(
        self, instance_id: Optional[str] = None, headless: bool = True, **kwargs
    ) -> BrowserInstance:
        """Create new browser instance (alias for create_instance for backward compatibility).

        Args:
            instance_id: Instance identifier (auto-generated if None)
            headless: Whether to run in headless mode
            **kwargs: Additional arguments passed to create_instance

        Returns:
            Browser instance
        """
        return await self.create_instance(
            instance_id=instance_id, headless=headless, **kwargs
        )
    ...
    async def _get_available_port(self) -> int:
        ...
        while True:
            ...
            in_use = any(
                instance.debugging_port == port for instance in self.instances.values()
            )
    ...
    **kwargs,
    ...
    async with BrowserManager(max_instances=1) as manager:
        instance = await manager.create_instance(
            headless=headless, window_size=window_size, profile=profile, **kwargs
        )
        try:
            yield instance
        finally:
