...
class AdvancedElementSelector:
    ...
    async def find_element(
        ...
        """
        logger.debug(f"Finding element with selector: {selector.type.value}={selector.value}")
    ...
    async def find_multiple_elements(
        ...
        logger.debug(f"Finding multiple elements: {selector.type.value}={selector.value}")
    ...
    async def _calculate_adaptive_timeout(
        ...
        logger.debug(f"Adaptive timeout: {adaptive_timeout:.1f}s (complexity: {complexity_score:.1f})")
