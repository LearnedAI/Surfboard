...
class WindowsAutomation:
    ...
    async def _find_windows_cross_platform(
        ...
        if self.platform == "linux":
            ...
            logger.warning(
                f"Window enumeration not implemented for platform: {self.platform}"
            )
                    ...
                    for window_id in result.stdout.strip().split("\n"):
                        if window_id.strip():
                            ...
                            windows.append(
                                WindowInfo(
                                    title=title,
                                    hwnd=int(window_id.strip())
                                    if window_id.strip().isdigit()
                                    else None,
                                )
                            )
        ...
        if self.is_windows:
            ...
            if self.platform == "linux":
                ...
                for tool in ["wmctrl", "xdotool"]:
                    try:
                        subprocess.run(
                            [tool, "--version"], capture_output=True, timeout=2
                        )
                        return True
                    except (
                        subprocess.TimeoutExpired,
                ...
                try:
                    subprocess.run(
                        ["osascript", "-e", "return 1"], capture_output=True, timeout=2
                    )
                    return True
                except (
                    subprocess.TimeoutExpired,
