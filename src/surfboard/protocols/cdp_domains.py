    ...
    async def navigate(self, url: str, wait_until: str = "load") -> Dict[str, Any]:
        ...
        if wait_until in ["load", "domcontentloaded"]:
            ...
            if wait_until == "load":
                ...
                self.client.add_event_handler("Page.domContentEventFired", on_load_event)
...
class RuntimeDomain(CDPDomain):
    ...
    async def get_properties(self, object_id: str) -> List[Dict[str, Any]]:
        ...
        """
        await self.enable()
        result = await self.client.send_command("Runtime.getProperties", {
            "objectId": object_id
        })
...
class DOMDomain(CDPDomain):
    ...
    async def query_selector(self, node_id: int, selector: str) -> Optional[int]:
        ...
        await self.enable()
        try:
            result = await self.client.send_command("DOM.querySelector", {
                "nodeId": node_id,
                "selector": selector
            })
    ...
    async def query_selector_all(self, node_id: int, selector: str) -> List[int]:
        ...
        await self.enable()
        try:
            result = await self.client.send_command("DOM.querySelectorAll", {
                "nodeId": node_id,
                "selector": selector
            })
    ...
    async def get_box_model(self, node_id: int) -> Optional[Dict[str, Any]]:
        ...
        await self.enable()
        try:
            return await self.client.send_command("DOM.getBoxModel", {
                "nodeId": node_id
            })
        except CDPError:
            return None
    ...
    async def get_attributes(self, node_id: int) -> Dict[str, str]:
        ...
        """
        await self.enable()
        result = await self.client.send_command("DOM.getAttributes", {
            "nodeId": node_id
        })
    ...
    async def set_attribute_value(self, node_id: int, name: str, value: str) -> None:
        ...
        """
        await self.enable()
        await self.client.send_command("DOM.setAttributeValue", {
            "nodeId": node_id,
            "name": name,
            "value": value
        })
...
class NetworkDomain(CDPDomain):
    ...
    async def set_user_agent_override(self, user_agent: str) -> None:
        ...
        """
        await self.enable()
        await self.client.send_command("Network.setUserAgentOverride", {
...
class EmulationDomain(CDPDomain):
    ...
    async def set_geolocation_override(
        ...
        """
        await self.enable()
        await self.client.send_command("Emulation.setGeolocationOverride", {
            "latitude": latitude,
            "longitude": longitude,
            "accuracy": accuracy
        })
