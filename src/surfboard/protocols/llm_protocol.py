...
class BrowserConfig(BaseModel):
    ...
    additional_args: List[str] = Field(
        default_factory=list, description="Additional Chrome args"
    )
...
class NavigateCommand(BaseCommand):
    ...
    command_type: Literal[CommandType.NAVIGATE] = Field(
        CommandType.NAVIGATE, description="Command type"
    )
...
class FindElementCommand(BaseCommand):
    ...
    command_type: Literal[CommandType.FIND_ELEMENT] = Field(
        CommandType.FIND_ELEMENT, description="Command type"
    )
...
class ClickCommand(BaseCommand):
    ...
    command_type: Literal[CommandType.CLICK] = Field(
        CommandType.CLICK, description="Command type"
    )
    selector: ElementSelector = Field(..., description="Element selector")
...
class TypeTextCommand(BaseCommand):
    ...
    command_type: Literal[CommandType.TYPE_TEXT] = Field(
        CommandType.TYPE_TEXT, description="Command type"
    )
    selector: ElementSelector = Field(..., description="Element selector")
...
class GetTextCommand(BaseCommand):
    ...
    command_type: Literal[CommandType.GET_TEXT] = Field(
        CommandType.GET_TEXT, description="Command type"
    )
    selector: ElementSelector = Field(..., description="Element selector")
...
class TakeScreenshotCommand(BaseCommand):
    ...
    command_type: Literal[CommandType.TAKE_SCREENSHOT] = Field(
        CommandType.TAKE_SCREENSHOT, description="Command type"
    )
    selector: Optional[ElementSelector] = Field(
        None, description="Element selector for clipping"
    )
...
class ExecuteScriptCommand(BaseCommand):
    ...
    command_type: Literal[CommandType.EXECUTE_SCRIPT] = Field(
        CommandType.EXECUTE_SCRIPT, description="Command type"
    )
...
class GetPageSummaryCommand(BaseCommand):
    ...
    command_type: Literal[CommandType.GET_PAGE_SUMMARY] = Field(
        CommandType.GET_PAGE_SUMMARY, description="Command type"
    )
...
class CreateBrowserCommand(BaseCommand):
    ...
    command_type: Literal[CommandType.CREATE_BROWSER] = Field(
        CommandType.CREATE_BROWSER, description="Command type"
    )
    browser_id: str = Field(..., description="Browser instance ID")
    config: BrowserConfig = Field(
...
class FindElementResponse(BaseResponse):
    ...
    elements: List[ElementInfo] = Field(
        default_factory=list, description="Found elements"
    )
...
class ScreenshotResponse(BaseResponse):
    ...
    dimensions: Dict[str, int] = Field(
        default_factory=dict, description="Image dimensions"
    )
...
class PageInfo(BaseModel):
    ...
    headings: List[Dict[str, str]] = Field(
        default_factory=list, description="Page headings"
    )
...
class ListBrowsersResponse(BaseResponse):
    ...
    browsers: List[BrowserInfo] = Field(
        default_factory=list, description="Active browsers"
    )
...
EXAMPLE_COMMANDS = {
    ...
    "find_element": {
        ...
        "selector": {"type": "css", "value": "button.submit", "timeout": 10.0},
    },
    "click": {"command_type": "click", "selector": {"type": "text", "value": "Submit"}},
    "type_text": {
