...
class PageAnalyzer:
    ...
    async def analyze_page(
        ...
        links = await self._extract_links(session, page_data["url"]) if include_links else []
        images = await self._extract_images(session, page_data["url"]) if include_images else []
    ...
    async def _analyze_page_elements(self, session: CDPSession, max_elements: int) -> List[ElementAnalysis]:
    ...
    async def _extract_links(self, session: CDPSession, base_url: str) -> List[Dict[str, str]]:
    ...
    async def _extract_images(self, session: CDPSession, base_url: str) -> List[Dict[str, str]]:
    ...
    async def get_interactive_elements(self, session: CDPSession, limit: int = 20) -> List[Dict[str, Any]]:
