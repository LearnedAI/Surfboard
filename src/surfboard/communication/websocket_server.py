...
class WebSocketServer:
    ...
    def __init__(
        ...
        self.browser_manager = BrowserManager(max_instances=max_clients * max_browsers_per_client)
    ...
    async def _process_message(self, session: ClientSession, message: str):
        ...
        try:
            ...
            if "command" in data:
                ...
                logger.warning(f"Unknown message type from {session.session_id}: {data}")
    ...
    async def _handle_command_message(self, session: ClientSession, data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"Executing command {command.command_type} for {session.session_id}")
            
            # Execute command
            start_time = time.time()
            response = await self.command_executor.execute_command(command, session.session_id)
            ...
            logger.debug(f"Command {command.command_type} completed in {execution_time:.2f}s")
        ...
        while self.running:
            try:
                ...
                for session_id in idle_sessions:
                    ...
                    session = self.clients[session_id]
                    try:
                        await session.websocket.close(code=1001, reason="Session timeout")
                    except:
                        pass
                    await self._cleanup_session(session_id)
